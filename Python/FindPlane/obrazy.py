...
def main():
    ...
    plt.subplots(xplots, yplots)
    plt.subplots_adjust(wspace = 0.1, hspace=0.1)
    for ktory, nazwa in enumerate(files):
        ax = plt.subplot(xplots,yplots,ktory)
        ax.axis('off')
        ...
        for component in zip(contours, hierarchy[0]):            
            ...
            if(len(contour)> maxlen/4):
                ...
                if(currHierarchy[3] == -1):
                    x_val = [x[0][0] for x in contour]
                    y_val = [x[0][1] for x in contour]
                    ax.plot(x_val, y_val, linewidth=1)    
                    circleX = sum(x_val) / len(x_val)
                    circleY = sum(y_val) / len(y_val)                    
                    cv2.circle(copy, (circleX, circleY), size,(255,255,255), thickness=size)
        plt.imshow(copy)
            
    #plt.show()    
    plt.savefig("colorplanes3.png", format='png', dpi=900)  
